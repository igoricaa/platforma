...
async function CoachForm() {
  try {
    ...
    const existingCoachUserIds = (await db.query.coaches.findMany()).map(
      (coach) => coach.userId
    );
    const eligibleUsers = availableUsers.filter(
      (user) => !existingCoachUserIds.includes(user.id)
    );
        ...
        {eligibleUsers.length === 0 ? (
          <div className='text-center py-6'>
            <p className='text-gray-500 mb-4'>
              No eligible users found with coach role.
            </p>
            <p className='text-sm text-gray-500'>
              First create a user with the coach role before creating a coach
              profile.
            </p>
            <div className='mt-4'>
              <Link
          ...
          <form action={createCoach} className='space-y-6'>
            ...
            <div>
              ...
              <p className='mt-1 text-sm text-gray-500'>
                Enter price in cents (e.g., 1999 for $19.99)
              </p>
            </div>
...
async function createCoach(formData: FormData) {
  ...
  const subscriptionPrice = parseInt(
    formData.get('subscriptionPrice') as string,
    10
  );
